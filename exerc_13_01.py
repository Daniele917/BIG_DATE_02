...
# Criando o array dos roubos de veículos
array_cvli = np.array(df_cvli["cvli"])

# Obtendo a média dos roubos de veículos
media_cvli = np.mean(array_cvli)

# Obtendo a mediana dos roubos de veículos
mediana_cvli = np.median(array_cvli)

# Obtendo a distância entre a média e a mediana dos roubos de veículos
distancia_cvli = abs((media_cvli - mediana_cvli) / mediana_cvli) * 100

# Obtendo o máximo e o mínimo dos roubos de veículos
maximo_cvli = np.max(array_cvli)
minimo_cvli = np.min(array_cvli)

# Obtendo a amplitude dos roubos de veículos
amplitude_cvli = maximo_cvli - minimo_cvli

# Obtendo os Quartis dos roubos de veículos - Método Weibull
...
# Posição 01: BoxPlot dos Crimes Violentos Letais Intencionais - CVLI
plt.subplot(2, 2, 1)
...
# Posição 02: Histograma dos Crimes Violentos Letais Intencionais - CVLI
plt.subplot(2, 2, 2)
plt.title('Histograma dos Crimes Violentos Letais Intencionais - CVLI')
plt.hist(array_cvli, bins=100, edgecolor='black', color='yellow')

# Posição 03: Ranking dos Batalhões de PM com Outliers Superiores
plt.subplot(2, 2, 3)
df_cvli_outliers_superiores_order = df_cvli_outliers_superiores.sort_values(by='cvli', ascending=True)
plt.title('Ranking dos Batalhões de PM com Outliers Superiores')
plt.barh(df_cvli_outliers_superiores_order['aisp'].astype(str), 
         df_cvli_outliers_superiores_order['cvli'], 
         color='purple')

# Posição 04: Medidas Descritivas dos Crimes Violentos Letais Intencionais - CVLI
plt.subplot(2, 2, 4)
